...
orig <- ggplot(df_combined, aes(x = x, color = Treatment)) +
  ...
  labs(y = "Fraction of total cells(A.U.)", x = "metagene(log2CPM)") +
  theme_minimal() +
  theme(
...
pdf_true_nt <- ggplot(df_combined[df_combined$Treatment=="Untreated",], aes(x = x, color = Treatment))+geom_density(bw = 0.05, size = 0.8)
...
a <- orig + scale_y_continuous(expand = c(0, 0), limits = c(0, scaled_pdf[['max']]),  breaks=scaled_pdf[['breaks']], labels=pdf_left_nt_data[['breaks']],
  ...
  labs(y = 'PDF for untreated cells', x = "Metagene")
a <- a + theme(
